...
makes_of_cars <- select(vehicles, make)

# Use the `distinct()` function to determine how many different car manufacturers
# are represented by the data set

nrow(distinct(vehicles, make, keep_all = F))
...
make_1997 <- arrange(made_1997, hwy)
...
worst_hwy_mpg <- filter(twd, hwy == min(hwy))

select(worst_hwy_mpg, id)
